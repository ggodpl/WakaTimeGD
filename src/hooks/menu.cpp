...
class $modify(MenuLayerHook, MenuLayer) {
    ...
    void promptOnline() {
		if (Mod::get()->hasSavedValue("online-prompt")) return promptAutoDownload(); // display auto-download prompt in case user didnt get to see that one for some reason
    ...
    void promptAutoDownload() {
		if (Mod::get()->hasSavedValue("auto-download-prompt")) return;
	...
	void showIosPopup() {
		if (Mod::get()->hasSavedValue("ios-info-popup")) return;

		auto alert = FLAlertLayer::create("WakaTime", "WakaTime CLI is not supported on iOS, which means you won't be able to sync your tracked time using WakaTime. You can still use local time tracking.", "Ok");
		
		alert->m_scene = this;
		alert->show();

		Mod::get()->setSavedValue("ios-info-popup", true);
	}
};